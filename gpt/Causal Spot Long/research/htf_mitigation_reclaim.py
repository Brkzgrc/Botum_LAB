...
"""4H mitigation-reclaim discovery, isolated from frozen r2.

An impulse is a *closed 4H* break above a prior 64H high. Its origin zone is
the latest bearish 4H candle before the break. A later 15M signal is allowed
only after price revisits that exact origin zone without a 4H close below it,
...
COOLDOWN_HOURS = 12
FAMILY = "HTF_MITIGATION_RECLAIM"


def candidate_rules():
    rules=[]; rid=0
    for impulse_ret in (4.0, 6.0, 8.0):
        for origin_lookback in (1, 2, 3):
            for max_age_h in (24, 48):
                rid += 1
                rules.append({"id":f"HMR_{rid:03d}", "family":FAMILY,
...
def rule_mask(z: pd.DataFrame, r: dict, old: pd.Series) -> pd.Series:
    ...
    retained=z.h4_close>=lo
    reclaim=(z.close>hi) & (z.close>z.prev_high_8) & (z.close_loc>=.60)
    retest_recent=touched.shift(1).rolling(32,min_periods=1).max().astype(bool)
...
def aggregate_main(indir,outdir):
    ...
    status="NO_STABLE_HTF_MITIGATION_RECLAIM"
...
def self_test():
    ...
    z=add_origin_features(base,z)
    required={"ior_40_1_origin_low","ior_60_2_origin_high","ior_80_3_age_h"}
