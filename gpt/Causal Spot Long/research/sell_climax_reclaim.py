...
"""Sell-climax absorption and reclaim discovery, isolated from frozen r2.

The causal path is a closed 1H high-volume sell climax, followed by a period
without a new low, then a 15M reclaim above the climax candle's high. This is
not a pullback-in-uptrend or breakout family; it tests absorption of an acute
downward liquidity event. Existing Price Families v2 events are excluded.
"""
...
COOLDOWN_HOURS = 12
FAMILY = "SELL_CLIMAX_RECLAIM"


def candidate_rules():
    rules=[]; rid=0
    for shock_ret in (-2.0, -4.0, -6.0):
        for climax_rvol in (1.5, 2.5, 4.0):
            for max_age_h in (4, 8):
                rid += 1
                rules.append({"id":f"SCR_{rid:03d}", "family":FAMILY,
                    "shock_ret":shock_ret, "climax_rvol":climax_rvol,
                    "max_age_h":max_age_h})
    return rules
...
def add_origin_features(base: pd.DataFrame, z: pd.DataFrame) -> pd.DataFrame:
    ...
    for shock_ret in (-2.0, -4.0, -6.0):
        for climax_rvol in (1.5, 2.5, 4.0):
            climax = (h1.ret_3h <= shock_ret) & (h1.rvol20 >= climax_rvol) & (h1.close_loc <= .45)
            p = np.arange(len(h1)); last = pd.Series(np.where(climax, p, np.nan), index=h1.index).ffill().to_numpy()
            ...
            tag=f"scr_{abs(int(shock_ret*10))}_{int(climax_rvol*10)}"
...
def rule_mask(z: pd.DataFrame, r: dict, old: pd.Series) -> pd.Series:
    tag=f"scr_{abs(int(r['shock_ret']*10))}_{int(r['climax_rvol']*10)}"
    lo=z[f"{tag}_low"]; hi=z[f"{tag}_high"]; age=z[f"{tag}_age_h"]
    # Reclaim must occur after the climax and no later 15M close may undercut it.
    retained=z.close.shift(1).rolling(16,min_periods=1).min()>=lo
    reclaim=(z.close>hi) & (z.close>z.prev_high_8) & (z.close_loc>=.60)
    return ((age>=1)&(age<=r["max_age_h"])&retained&reclaim
...
def aggregate_main(indir,outdir):
    ...
    status="NO_STABLE_SELL_CLIMAX_RECLAIM"
...
def self_test():
    ...
    z=add_origin_features(base,z)
    required={"scr_20_15_low","scr_40_25_high","scr_60_40_age_h"}
