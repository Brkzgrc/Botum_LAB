...
def viable_floor(md,mc):
    return (
        md.get("n",0)>=10 and mc.get("n",0)>=8 and
        md.get("win24",0)>=55 and mc.get("win24",0)>=55 and
        md.get("mean24",-99)>0.5 and mc.get("mean24",-99)>0.5 and
        md.get("mean72",-99)>0 and mc.get("mean72",-99)>0 and
        md.get("mean168",-99)>0 and mc.get("mean168",-99)>0 and
        (md.get("pf24") or 0)>1.2 and (mc.get("pf24") or 0)>1.2
    )
...
def main(indir,outdir):
        ...
        for oh in PROTO_H:
            ...
            for q in [.70,.75,.80,.85,.875,.90,.925,.95,.965]:
                ...
                rec={"name":nm,"lookback_h":lb,"prototype_outcome_h":oh,"q":q,"threshold":th,
                     "discovery":md,"calibration":mc,"viable":viable_floor(md,mc)}
                candidates.append(rec); masks[nm]=m

    viable=[r for r in candidates if r["viable"]]
    def rank(r):
        d0=r["discovery"]; c0=r["calibration"]
        return min(d0["mean24"],c0["mean24"])+0.02*min(d0["win24"],c0["win24"])+0.15*min(d0["pf24"],c0["pf24"])+0.15*min(d0["mean72"],c0["mean72"])+0.08*min(d0["mean168"],c0["mean168"])
    viable=sorted(viable,key=rank,reverse=True)
    ...
    for rec in viable:
        ...
        if not viable_floor(md,mc): continue
        if mx.get("n",0)>=20 and (mx.get("mean24",-99)<=0 or mx.get("mean72",-99)<=0): continue
    ...
    summary={
      ...
      "top_viable":[{"name":r["name"],"lookback_h":r["lookback_h"],"prototype_outcome_h":r["prototype_outcome_h"],"discovery":r["discovery"],"calibration":r["calibration"],"cross_pre2026":metrics(good[cross&masks[r["name"]]]),"all_2026":metrics(good[all26&masks[r["name"]]])} for r in viable[:10]]
    }
    pd.DataFrame([{
      "name":r["name"],"lookback_h":r["lookback_h"],"prototype_outcome_h":r["prototype_outcome_h"],"q":r["q"],"threshold":r["threshold"],"viable":r["viable"],
    ...
    (outdir/"REPORT.md").write_text("# Phase 8 Multi-Window Trajectory Families\n\n"+json.dumps(summary,indent=2,ensure_ascii=False),encoding="utf-8")
