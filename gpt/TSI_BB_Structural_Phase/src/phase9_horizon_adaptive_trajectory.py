...
def main(indir,outdir):
    ...
    for r in viable:
        r["best_horizon"]=max(r["viable_horizons"],key=lambda h:horizon_score(r,h))
    ...
    for rec in viable:
        ...
        if mx.get("n",0)>=20 and mx.get(f"mean{h}",-99)<=0: continue
    ...
    summary={
      ...
      "top_viable":[{"name":r["name"],"lookback_h":r["lookback_h"],"prototype_outcome_h":r["prototype_outcome_h"],"best_horizon":r["best_horizon"],"discovery":r["discovery"],"calibration":r["calibration"],"cross_pre2026":metrics(good[cross&masks[r["name"]]]),"all_2026":metrics(good[all26&masks[r["name"]]])} for r in viable[:10]]
    }
    pd.DataFrame([{
    ...
    (outdir/"REPORT.md").write_text("# Phase 9 Horizon-Adaptive Trajectory Families\n\n"+json.dumps(summary,indent=2,ensure_ascii=False),encoding="utf-8")
