...
def main(indir,outdir):
    ...
    # Phase12 is scientifically distinct from Phase11: joint/intersection regimes, not one-dimensional regime buckets.
    regimes={
      'DEEP_BTCBB_HIGH':deep&bh,'DEEP_BTCBB_LOW':deep&bl,'MID_BTCBB_HIGH':mid&bh,'MID_BTCBB_LOW':mid&bl,
      'FLAT_BTCBB_HIGH':flat&bh,'FLAT_BTCBB_LOW':flat&bl,'DEEP_ALTBB_HIGH':deep&ah,'DEEP_ALTBB_LOW':deep&al,
      'MID_ALTBB_HIGH':mid&ah,'MID_ALTBB_LOW':mid&al,'BTCBB_HIGH_ALTBB_LOW':bh&al,'BTCBB_LOW_ALTBB_HIGH':bl&ah,
      'DOUBLE_HIGH':bh&ah,'DOUBLE_LOW':bl&al,
    }
    cand=[]; masks={}
    for rn,rg in regimes.items():
      for lb in p10.LOOKBACKS:
       for oh in H:
        ...
        score,_,_,_=built; base=pd.Series(score,index=g.index); sd=base[disc&rg&pool].dropna()
        if len(sd)<25: continue
        for q in [.60,.65,.70,.75,.80,.85,.875,.90,.925,.95]:
          ...
          sc=min(utility(md,bhh),utility(mc,bhh),utility(mx,bhh)); name=f'{rn}_LB{lb}_P{oh}_Q{int(q*1000)}'
    ...
    out={'method':'Joint/intersection causal regime trajectory reverse engineering with payoff-aware WR/mean/median/PF utility. Selection is PRE-2026 D+C+cross only; 2026 sealed until final evaluation.','selection_uses_2026':False,'events':len(g),'errors':len(errs),'robust_candidates':len(cand),'selected':selected,'r2_2026':met(g[y26&r2]),'expanded_2026':met(g[y26&union]),'signals_per_week_2026':int((y26&union).sum())/weeks,'top_candidates':[{**c,'all_2026':met(g[y26&masks[c['name']]])} for c in cand[:15]]}
    (outdir/'summary.json').write_text(json.dumps(out,indent=2,ensure_ascii=False),encoding='utf-8')
    (outdir/'REPORT.md').write_text('# Phase 12 Intersection-Regime Trajectory\n\n```json\n'+json.dumps(out,indent=2,ensure_ascii=False)+'\n```\n',encoding='utf-8')
