...
def close_position(p,bar,now):
    """Live payload exit contract: TP1 öncesi stop/24s expiry; TP1 sonrası ATR trailing, entry floor."""
    ...
    if p["tp1_hit"]:
        atr=float(p.get("atr_entry") or 0.0)
        trail=max(p["entry"],p["peak"]-0.6*atr) if atr>0 else p["entry"]
...
def main():
    a=argparse.ArgumentParser(description="Live legacy v11 baseline replay")
    a.add_argument("--start",default="2026-01-01"); a.add_argument("--end",default=pd.Timestamp.now(tz="UTC").strftime("%Y-%m-%d"))
    a.add_argument("--symbols",type=int,default=0,help="0=tum arastirma evreni; pozitif deger ilk N pariteyi sinirlar"); a.add_argument("--output",default="legacy_baseline.json")
      ...
      for n,ts in enumerate(steps,1):
        ...
        try:
            ...
            for c in sorted(finals,key=lambda x:x.rank,reverse=True)[:room]:
                ...
                positions[c.symbol]={"entry_time":ts.isoformat(),"entry":entry,"stop":stop,"tp1":tp1,"peak":entry,"tp1_hit":False,"expiry":ts+pd.Timedelta(hours=24),"atr_entry":float(c.snapshot["1h"].get("atr_pct",0))*entry/100.0,"setup_kind":c.decision["setup_kind"],"score":round(float(c.decision["confidence"]),2),"btc_regime":regime,"features":feats}
