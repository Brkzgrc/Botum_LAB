...
### PRESSURE zaman-akışlı replay ön denetimi — hazırlık
...
### PRESSURE çok-dönem olay yoğunluğu ön denetimi — hazırlık

Tek SOL hareketine bağlı 36 saatlik sonucun genellenmesini önlemek için 2025'in dört sabit takvim penceresi önceden kilitlendi: 15–16 Şubat, 15–16 Mayıs, 15–16 Ağustos ve 15–16 Kasım; her biri 36 saat. Önceki 28–30 Ağustos sonucu bu seçime dahil edilmedi. Evren bu ön denetimde 12 likit Spot USDT sembolü: BTC, ETH, SOL, XRP, LINK, AVAX, BNB, DOGE, ADA, TRX, LTC, SUI; stable/fiat/leveraged base yok. Sinyalsiz pencere hata değil, gerçek olay yoğunluğu olarak tutulacak.

Her pencere ayrı shard; 1D/4H/1H/15M yalnız kapanmış mum, geçmiş 24 saat quote-volume, watch-state, rank ve günlük üç legacy kota korunur. Her olay için 24 saatlik 5M MFE/MAE ve mevcut hedef/stop ilk-geçiş matrisi kaydedilir. Aggregate 0/6/12/24/48 saat aynı-coin cooldown ablation'ı yapar; bu aşamada eşik/champion seçmez. Maliyet %0,20; aynı 5M mum hedef+stop çakışması stop-first.

Önceki 6 sembollü çalışma 4 dk 04 sn sürdü. 12 sembol × dört paralel pencere için tahmini toplam 32–48 runner-dk, 10–18 dk duvar süresi; her shard job 22 dk, veri komutu 17 dk, aggregate 10 dk ile sınırlı. 4/4 shard, her shard 12/12 sembol ve en az 100 tarama zorunlu; eksik shard aggregate'i başarısız yapar ve hata artifact'i korunur. Derleme+self-test, AST/UTC/nedensellik, YAML ve 10 kabuk bloğu, sentetik 4/4 aggregate/cooldown testi ayrı ayrı geçti. Sentetik test üreticisindeki geçersiz ay metni workflow başlamadan fark edilip düzeltildi; üretim kodunda değişiklik gerektirmedi. Sonuç okunmadan pahalı tam evren araştırması başlamayacak; `Clear System.py` değişmedi.


### PRESSURE çok-dönem ön denetimi — teknik hata run 36257930090

[GitHub Actions run 36257930090](https://github.com/Brkzgrc/Botum_LAB/actions/runs/36257930090) 2026-09-26 17:08:04–17:17:34 UTC arasında `completed/failure`; yaklaşık 9 dk 30 sn duvar ve yaklaşık 22–26 runner-dk. Preflight compile/self-test/static adımları geçti. Q1, 2025-01-01 başlangıcından 15 Şubat'a kadar kaynak tarayıcının zorunlu 80 kapanmış 1D mumunu üretemediği için `BTCUSDT 1d: incomplete/insufficient historical bars` ile erken durdu. Q2–Q4 veri/replay'i tamamladıktan sonra kodda altı sembollü ilk smoke'dan kalmış `len(coverage) != 6` sabiti, yeni 12 sembollü shardları yanlışlıkla `incomplete replay shard` saydı. Aggregate 0/4 geçerli summary görüp beklendiği gibi `INCOMPLETE_SHARDS` ile başarısız oldu. Dört shard ve aggregate log artifact'leri korundu.
...
## PRESSURE çevrimiçi cooldown/kota ikamesi hazırlığı — 2026-09-26

Önceki çok-dönem preflight'taki cooldown sonuçları, sinyalleri sonradan listeden çıkardığı için günlük üç aday kotasında boşalan yere aynı taramadaki sıradaki adayın girip girmediğini modellemiyordu. Bu boşluğu kapatmak için `pressure_replay_preflight.py`, cooldown'u sıralanmış `ALIM_ADAYI` listesinin günlük kota kesiminden **önce** uygulayacak biçimde genişletildi. Böylece 0/6/24/48 saat varyantlarında çıkarılan olaylar ve gerçekten oluşan ikame olaylar `(symbol, decision_time)` kimliğiyle karşılaştırılacak.
...
## PRESSURE path-dependent çıkış parity hazırlığı — 2026-09-26

Full-market eşik araştırmasına geçmeden önce kaynak çıkış semantiğinde önemli bir fark doğrulandı: önceki replay'deki `source_tp1`, TP1'i nihai satış sayıyordu; arşivlenen scanner/tracker ise TP1'i kapanış değil trailing aktivasyonu olarak kullanıyor. Gerçek paper kuralı TP1 öncesi yapısal stop ve 24 saat inceleme ufku; TP1 sonrası giriş altına inmeyen ATR(14, 1H) × 0,6 trailing ve artık süre sınırı olmaması. Yeni `pressure_path_exit.py`, nihai sistemin kapalı-mum şartını korumak için ATR'yi yalnız son tamamen kapanmış 1H mumlardan hesaplıyor ve %0,20 round-trip maliyet uyguluyor.

Aynı tarihsel sinyallerde on nedensel yol politikası karşılaştırılacak: düzeltilmiş kaynak ATR trailing; +%0,50 veya +%0,80 görüldükten sonra brüt +%0,20 taban; MFE +%1 sonrası %0,50 geri-verme ve +%0,20 taban; %50'si +%0,50'de, kalanı +%1,50/tabanda kademeli çıkış; yalnız önce +%0,50 hareket görüldükten sonra iki kapalı 15M boyunca fiyat+MACD momentum+hacim/OBV sönmesi; 6/12/24/48 saat zorunlu kapanış. Stop ile aktivasyon aynı 5M mumdaysa ihtiyatlı stop-first uygulanır; 72 saatte hâlâ açık kaynak-trailing kayıtları ayrıca `CENSORED` sayılır ve kapanmış sonuçlarla karıştırılmaz.

Sabit kapsam dört ayrı 2025 penceresi × 12 stable/fiat olmayan Binance Spot USDT sembolü = toplam 6 gözlem günü. Tahmini bütçe **40–55 runner-dk / 10–16 dk duvar süresi**; preflight 10 dk, replay job 25 dk/komut 19 dk, aggregate 10 dk. Derleme ve üç self-test; AST/YAML/bash; arşivlenmiş kaynak sözleşmesi; look-ahead/POST yasağı; tamamlanmış ve eksik sentetik shard aggregate kontrolleri ayrı ayrı geçti. Bu bir mekanizma preflight'ıdır; eşik seçimi/OOS/full-market/sermaye slotu değildir. `Clear System.py` değişmedi. Sonraki somut adım: bu smoke yalnız hangi çıkış mekanizmalarının pahalı full-market Discovery/Calibration araştırmasına taşınacağını belirleyecek.


### Path-exit ilk teknik run — 36264996472

Run [36264996472](https://github.com/Brkzgrc/Botum_LAB/actions/runs/36264996472) araştırma shard'ları başlamadan preflight'ta güvenli biçimde durdu. Compile ve üç deterministik self-test geçti; hata statik denetimdeki aşırı katı metin aramasıydı: kodda geçerli `hours: int = 72` ve `pd.Timedelta(hours=72)` bulunmasına rağmen workflow yalnız bitişik `hours=72` metnini iki dosyada birden arıyordu. Bu bir model/veri/çıkış motoru hatası değildir. Replay job’ları skipped kaldı; aggregate beklendiği gibi eksik shard nedeniyle başarısız oldu; pahalı tarama maliyeti oluşmadı.

Aynı run yeniden çalıştırılmadı. Denetim semantik olarak iki gerçek sözleşmeyi ayrı doğrulayacak biçimde düzeltildi. Düzeltme sonrası (1) exact AST/static kontrolü, (2) YAML+bash+timeout/artifact kontrolü, (3) arşivlenen scanner/tracker kaynak sözleşmesi ve üç self-test tekrar geçti. Sonraki somut adım, yalnız düzeltilmiş workflow commit’inin otomatik run’ını izlemek ve summary oluşursa metrikleri incelemektir. `Clear System.py` değişmedi.
