...
| 2026-09-22 | Taker Flow Absorption/Release run 35674885928 | ÇALIŞIYOR | 15M taker dengesindeki emilim, akış dönüşü ve serbestleşme; kapanmış 1H/4H akış dizileriyle 26 özellik olarak test ediliyor. | Bütçe: yaklaşık 730–900 runner-dk / 50–65 dk duvar süresi; maliyet %0,20. Yerel compile, self-test, AST/nedensellik ve YAML kontrolleri geçti. Workflow compile/self-test/statik denetim geçti; 6 sembollük gerçek-veri smoke sürüyor. Sinyal/gün, active-day, expectancy, PF, target/stop-first, MFE/MAE ve OOS: sonuç bekleniyor. `Clear System.py` değiştirilmedi. |
...
Run `35674885928` şu anda tek pahalı araştırma olarak çalışıyor. Compile, deterministik self-test ve statik nedensellik/evren denetimi geçti; altı sembollük gerçek-veri smoke tamamlanmadan shard'lar açılmayacak. Sonraki turda önce bu run izlenecek; yeni araştırma başlatılmayacak. Başarılı olursa `summary.json` içindeki tüm dönem metrikleri okunacak. Aday ancak 2026 OOS'u geçip frozen r2 ile dedupe/OR kalite doğrulamasını da başarırsa `Clear System.py` içine alınacak.
