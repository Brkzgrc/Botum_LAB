...
| 2026-09-26 | Market Breadth Ignition run 36235398705 | TEKNİK HATA / SHARDLAR KORUNDU | Tam context, compile/self-test, nedensellik/evren denetimi, iki-coin gerçek-veri smoke ve 64/64 hedef shard başarıyla geçti; aggregate dört tarihçesiz yeni sembolü yanlışlıkla hard error saydı. | `completed/failure`; 36 dk 37 sn duvar süresi; veri hedefi 2023-01-01–2026-09-18; maliyet %0,20. Kesin hata: HYPEUSDT, AMCBUSDT, AGPUBUSDT, CYPHBUSDT için `RuntimeError:no 15m data`. Bunlar shard/evren bütünlüğü bozulmadan sınırlı tarihçe boşluğu olarak düzeltilmiştir. Henüz `summary.json` oluşmadığından sinyal/gün, active-day, expectancy, PF, target/stop-first, MFE/MAE ve OOS beklemede; `Clear System.py` değiştirilmedi. |
...
Run `36237687553` aggregate-kurtarma olarak queued durumunda. Kaynak run `36235398705` içindeki 64/64 başarılı shard artifact'i yeniden kullanılacak; yeni pahalı shard taraması yapılmayacak. Düzeltme Python compile/self-test, AST/statik sözleşme, workflow şema-güvenlik, hata-artifact ve 64-shard sentetik regresyon kontrollerinden geçti. Sonraki tur yalnız bu run'ı izleyecek; tamamlanırsa `summary.json` ve `aggregate_diagnostics.json` içindeki veri dönemi, maliyet, frekans, active-day, expectancy, PF, target/stop-first, MFE/MAE ve OOS metriklerini okuyup kararı kaydedecek. Doğrulanmış champion ve ayrıca r2 OR doğrulaması olmadan `Clear System.py` değişmeyecek.
