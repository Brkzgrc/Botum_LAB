...
| 2026-09-26 | Market Breadth Ignition run 36235398705 | KUYRUKTA | Ortak BTC/ETH/BNB/SOL/XRP/DOGE breadth daralma→genişleme hareketinden sonraki coin-bazlı kapalı-mum yapı kırılımı; 25 breadth+yerel hareket özelliği, pre-2026 cross-holdout kapısıyla araştırılıyor. | Bütçe: 350–430 runner-dk / 45–55 dk; maliyet %0,20. Yerel compile/self-test, AST-look-ahead/holdout, YAML-timeout-artifact ve sentetik entegrasyon geçti. Workflow önce tam context + iki-coin gerçek-veri smoke çalıştıracak; bunlar geçmeden 64 shard açılmayacak. Performans/OOS bekleniyor; `Clear System.py` değiştirilmedi. |
...
Run `36235398705` Market Breadth Ignition queued/başlangıç durumunda ve tek pahalı araştırmadır. Sonraki kontrol önce compile/self-test/statik denetim, tam-period context ve iki-coin smoke adımlarını izleyecek; yeni çalışma başlatılmayacak. Bunlar geçerse 64 shard başlayacak. Run tamamlandığında yalnız yeşil duruma bakılmayacak; `summary.json` içindeki Discovery, Calibration, CROSS_HOLDOUT_PRE2026, 2026, frekans, active-day, expectancy, PF, target/stop-first ve MFE/MAE okunacak. R2 OR doğrulaması geçmeden kanonik sistem değişmeyecek.
